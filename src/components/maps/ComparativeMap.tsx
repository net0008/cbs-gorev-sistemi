  ...
  return (
    <div className="h-full w-full min-h-[400px]">
      <MapContainer center={[39.12, 27.18]} zoom={13} className="h-full w-full relative z-0">
        ...
        {referenceGeoJSON && (
          <GeoJSON data={referenceGeoJSON} style={() => ({ color: '#3b82f6', weight: 4, opacity: 0.6, dashArray: '5, 5' })} />
        )}
        
        {/* Öğrencinin Çizimi (Kırmızı) */}
        {studentGeoJSON && (
          <GeoJSON data={studentGeoJSON} style={() => ({ color: '#ef4444', weight: 4, opacity: 0.8 })} />
        )}
      </MapContainer>
    </div>
  );
