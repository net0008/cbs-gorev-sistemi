...
function MeasureTab() {
  ...
  const [realKm, setRealKm]     = useState(0);
  const [knownReal, setKnownReal] = useState<number|undefined>(undefined);
  const mapContainerRef = useRef<HTMLDivElement>(null);
  const mapRef          = useRef<any>(null);
  const markersRef      = useRef<any[]>([]);
  const lineRef         = useRef<any>(null);
  const selScaleRef     = useRef(selScale);
  selScaleRef.current   = selScale;

  // Leaflet dinamik yükleme
  useEffect(() => {
  ...
  const cityARef  = useRef<City|null>(null);
  const cityBRef  = useRef<City|null>(null);
  const stepRef   = useRef<"pickA"|"pickB"|"result">("pickA");

  const handleCityClick = (city: City) => {
    sndClick();
    if (stepRef.current === "pickA") {
      cityARef.current = city; cityBRef.current = null;
      stepRef.current = "pickB";
      setCityA(city); setCityB(null); setStep("pickB");
      updateMarkers();
    } else if (stepRef.current === "pickB" && city.id !== cityARef.current?.id) {
      cityBRef.current = city;
      stepRef.current = "result";
      setCityB(city); setStep("result");
      const key = `${cityARef.current?.id}-${city.id}`;
      const km  = REAL_KM[key];
      setRealKm(km || 0);
      setKnownReal(km);
      updateMarkers();
      drawLine();
  ...
  const reset = () => {
    cityARef.current = null; cityBRef.current = null; stepRef.current = "pickA";
    setCityA(null); setCityB(null); setStep("pickA"); setRealKm(0); setKnownReal(undefined);
    if (lineRef.current && mapRef.current) { lineRef.current.remove(); lineRef.current = null; }
    updateMarkers();
    sndClick();
  };
  ...
  const updateMarkers = () => {
    ...
    markersRef.current.forEach(m => m.setStyle({
      color: m._cityId === cityARef.current?.id ? "#10b981"
           : m._cityId === cityBRef.current?.id ? "#fbbf24"
           : "#ef4444",
      fillColor: m._cityId === cityARef.current?.id ? "#10b981"
              : m._cityId === cityBRef.current?.id ? "#fbbf24"
              : "#ef4444",
      weight: (m._cityId === cityARef.current?.id || m._cityId === cityBRef.current?.id) ? 3 : 1.5,
      radius: (m._cityId === cityARef.current?.id || m._cityId === cityBRef.current?.id) ? 9 : 6,
    }));
  };

  const drawLine = () => {
    const L = (window as any).L;
    if (!L || !mapRef.current || !cityARef.current || !cityBRef.current) return;
    if (lineRef.current) lineRef.current.remove();
    const [lonA, latA] = [CITIES.find(c=>c.id===cityARef.current!.id)!.lon, CITIES.find(c=>c.id===cityARef.current!.id)!.lat];
    const [lonB, latB] = [CITIES.find(c=>c.id===cityBRef.current!.id)!.lon, CITIES.find(c=>c.id===cityBRef.current!.id)!.lat];
    lineRef.current = L.polyline([[latA,lonA],[latB,lonB]], {
      color:"#fbbf24", weight:2.5, dashArray:"8,5", opacity:0.9
  ...
  const initMap = () => {
    ...
    style.textContent = `
      .leaflet-city-tooltip {
        background:#1a1000;color:#fbbf24;border:1px solid #d97706;
    ...
    <div style={{ flex:1, display:"flex", overflow:"hidden" }}>
      ...
      <div style={{ width:"320px", flexShrink:0, borderRight:"1px solid rgba(251,191,36,0.1)", background:C_PANEL, overflowY:"auto", padding:"26px", display:"flex", flexDirection:"column", gap:"16px" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
          {[{n:"1",t:"Yukarıdan bir ölçek seçin"},{n:"2",t:"Haritada ilk şehre tıklayın"},{n:"3",t:"İkinci şehre tıklayın"},{n:"4",t:"Gerçek mesafeyi okuyun"}].map(s=>(
        ...
        <SDivider />
        {step==="pickA"&&<InfoBox><div style={{ fontSize:"15px",color:C_GOLD,fontWeight:"700",fontFamily:FONT_BODY }}>📍 İlk şehri seçin</div><div style={{ fontSize:"13px",color:"#5a4010",marginTop:"5px",fontFamily:FONT_BODY }}>Haritada herhangi bir şehre tıklayın.</div></InfoBox>}
        {step==="pickB"&&<InfoBox accent={C_GREEN}><div style={{ fontSize:"15px",color:C_GREEN,fontWeight:"700",fontFamily:FONT_BODY }}>📍 {cityA?.name} seçildi</div><div style={{ fontSize:"13px",color:"#0d5a38",marginTop:"5px",fontFamily:FONT_BODY }}>Şimdi ikinci şehri seçin.</div></InfoBox>}
        ...
        <div style={{ marginTop:"auto", padding:"10px 12px", background:"rgba(0,0,0,0.2)", borderRadius:"7px", fontSize:"11px", color:"#3a2a08", lineHeight:"1.6", fontFamily:FONT_BODY }}>
          📍 {CITIES.length} il · OpenStreetMap · KGM mesafeler
        </div>
      </div>

      {/* Leaflet harita */}
      <div style={{ flex:1, position:"relative", background:"#1a2535" }}>
