...
/**
 * 1. BECK ET AL. (2023) - TÜM 30 İKLİM SINIFI
 */
const climateLegend: Record<number, { code: string; name: string; color: string }> = {
  ...
  9: { code: 'Csb', name: 'Temperate, dry/warm summer', color: 'rgb(200, 200, 0)' },
  10: { code: 'Csc', name: 'Temperate, dry/cold summer', color: 'rgb(150, 150, 0)' },
...
function ComparisonControl({ layerLeft, layerRight }: { layerLeft: any, layerRight: any }) {
  ...
  return (
    <style dangerouslySetInnerHTML={{ __html: `
      ...
      /* Slider tutma kolu - Chrome/Safari */
      .leaflet-sbs-range::-webkit-slider-thumb {
        -webkit-appearance: none;
        width: 40px;
        height: 40px;
        background: #fff url('https://cdn-icons-png.flaticon.com/512/32/32213.png') no-repeat center;
        background-size: 60%;
        border-radius: 50%;
      ...
      <div className="flex items-center justify-between p-4 border-b border-white/10 bg-slate-900/90 backdrop-blur-md">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-blue-400">1930 (Mevcut) vs 2099 (Projeksiyon)</p>
          </div>
        </div>
        <div className="flex items-center gap-2 bg-blue-500/20 px-3 py-1 rounded-full border border-blue-500/30 text-blue-400 text-xs animate-pulse">
           <Globe size={14} /> Karşılaştırma Modu Aktif
        </div>
      </div>

      <div className="flex-1 relative bg-slate-900">
        {loading && (
          <div className="absolute inset-0 z-[11000] bg-slate-950 flex flex-col items-center justify-center gap-4">
            <Loader2 className="w-12 h-12 text-blue-500 animate-spin" />
            <p className="text-slate-300 font-medium tracking-widest uppercase text-xs">Raster Veriler İşleniyor...</p>
          </div>
        )}
        ...
        {/* Legend (Gösterge) - Sol Altta, Kaydırılabilir */}
        <div className="absolute bottom-6 left-6 z-[2000] w-64 bg-slate-900/95 border border-white/10 rounded-2xl flex flex-col shadow-2xl max-h-[60%] overflow-hidden backdrop-blur-md">
          <div className="p-3 bg-blue-500/10 border-b border-white/5 flex items-center justify-between">
            <span className="text-[10px] font-bold text-blue-400 uppercase">İklim Göstergesi</span>
