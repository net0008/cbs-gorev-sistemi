              ...
              {sec==="insanfaktor" && (
                <div style={{animation:"fadeUp 0.22s ease",display:"flex",flexDirection:"column",gap:"13px"}}>
                  ...
                  <div style={{padding:"13px 16px",background:"rgba(124,58,237,0.06)",border:"1px solid rgba(124,58,237,0.15)",borderRadius:"9px"}}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:"5px"}}>
                      {[["Enerji Tedarik Sektörü",33,"#dc2626"],["Sanayi",24,"#7c3aed"],["Tarım-Ormancılık",22,"#166534"],["Ulaşım",15,"#0369a1"],["Konutlar",6,"#f59e0b"]].map(([label,pct,color]: [string, number, string])=>(<div key={String(label)} style={{display:"flex",gap:"8px",alignItems:"center"}}><div style={{width:"150px",fontSize:"12px",color:"#94a3b8",fontFamily:FONT,flexShrink:0}}>{String(label)}</div><div style={{flex:1,background:`${String(color)}18`,borderRadius:"3px",height:"12px",overflow:"hidden"}}><div style={{height:"100%",width:`${pct*2.5}%`,background:String(color),borderRadius:"3px"}}/></div><div style={{width:"35px",fontSize:"12px",fontWeight:"800",color:String(color),fontFamily:MONO,textAlign:"right"}}>%{pct}</div></div>))}
                    </div>
                  </div>
                </div>
              )}
