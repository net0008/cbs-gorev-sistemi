...
/* ─── RasterLayer: key prop sayesinde her yıl değişiminde yeniden mount edilir ─── */
function RasterLayer({ georaster }: { georaster: any }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Önce varsa eski katmanı kaldır
      if (layerRef.current) {
...
export default function IklimTurleriActivity({ onClose }: { onClose: () => void }) {
  ...
  // Önceki isteği iptal etmek için ref
  const abortRef = useRef<AbortController | null>(null);

  const loadRaster = useCallback(async (year: number) => {
    // Aynı yıl tekrar tıklanmışsa hiçbir şey yapma
    if (year === activeYear && currentRaster) return;
    ...
    try {
      ...
      // Cache-busting: her istek için benzersiz URL
      const url = `/maps/climate/${year}Koppen_geiger.tif?_=${year}`;
      const response = await fetch(url, {
        signal: controller.signal,
        cache: 'no-store',          // ← tarayıcı önbelleğini atla
      });

      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: ${year} verisi alınamadı`);
      }
      ...
      // Önce eski raster'ı null yap, sonra yenisini ata
      // Bu, RasterLayer'ın key değişimiyle tamamen yeniden mount edilmesini sağlar
      setCurrentRaster(null);
      // Bir tick bekle, ardından yeni raster'ı set et
      ...
      if (err.name === 'AbortError') return; // kullanıcı başka yıla geçti, sessizce iptal
      console.error('Raster yükleme hatası:', err);
      setError(err.message ?? 'Bilinmeyen hata');
    } finally {
      setLoading(false);
    }
  }, [activeYear, currentRaster]);

  // İlk yüklemede 1930'u çek
  useEffect(() => {
    loadRaster(1930);
    ...
    <div className="fixed inset-0 z-[10000] flex flex-col bg-slate-950 text-slate-100 animate-in fade-in duration-300 overflow-hidden">
      ...
      <div className="flex-1 relative bg-slate-900">
        ...
        >
          ...
          {/*
            key={activeYear} → yıl her değiştiğinde RasterLayer tamamen yeniden
            mount edilir; bu, georaster değişiminin kesinlikle algılanmasını sağlar.
          */}
          {currentRaster && (
            <RasterLayer key={activeYear} georaster={currentRaster} />
          )}
        </MapContainer>
        ...
        <div className="absolute bottom-6 right-6 z-[2000] w-72 md:w-80 bg-slate-950/90 border border-white/10 rounded-2xl flex flex-col shadow-2xl max-h-[75%] backdrop-blur-md">
          ...
          <div className="p-3 overflow-y-auto space-y-2">
            {Object.entries(climateLegend).map(([k, v]) => (
              <div
                key={k}
                className="flex items-center gap-3 hover:bg-white/5 p-1 rounded-lg transition-colors group"
              >
                <div
                ...
                <div className="flex flex-col min-w-0">
                  <span className="text-[10px] font-bold text-slate-200 group-hover:text-white">
                    {v.code}
                  </span>
